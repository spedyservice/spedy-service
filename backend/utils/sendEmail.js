...
class EmailService {
  ...
  async sendOrderConfirmation(order, customerEmail) {
    ...
    const html = `
      ...
      <style>body{font-family:Arial,sans-serif;line-height:1.6;color:#333}.container{max-width:600px;margin:0 auto;padding:20px}.header{background:#2563eb;color:white;padding:30px 20px;text-align:center}.content{padding:30px 20px;background:#f9fafb}.order-details{background:white;padding:20px;margin:20px 0;border-left:4px solid #2563eb}.status{display:inline-block;padding:5px 12px;background:#f59e0b;color:white;border-radius:5px;font-size:12px}table{width:100%;border-collapse:collapse}th{text-align:left;padding:8px;background:#f3f4f6}.footer{text-align:center;padding:20px;font-size:12px;color:#6b7280;border-top:1px solid #e5e7eb}.total-row{font-weight:bold}.detail-row{margin-bottom:10px}.detail-label{font-weight:bold;width:120px;display:inline-block}</style>
      </head>
      <body>
        <div class="container"><div class="header"><h1>Spedy Service</h1><p>Order Confirmation</p></div>
        <div class="content">
          ...
          <div class="order-details"><h3>Order Details:</h3>
          ...
          <div class="detail-row total-row"><span class="detail-label">Total:</span> ₹${order.totalPrice}</div></div>
          <h3>Shipping Address</h3><p>${order.shippingAddress?.fullName}<br/>${order.shippingAddress?.address}<br/>${order.shippingAddress?.city}, ${order.shippingAddress?.postalCode}<br/>${order.shippingAddress?.phone}</p>
  ...
  async sendOrderStatusUpdate(order, customerEmail, oldStatus, newStatus) {
    ...
    const html = `
      <!DOCTYPE html>
      <html>
      <head><meta charset="UTF-8"><title>Order Status Update</title>
      <style>body{font-family:Arial,sans-serif;line-height:1.6;color:#333}.container{max-width:600px;margin:0 auto;padding:20px}.header{background:#2563eb;color:white;padding:20px;text-align:center}.content{padding:20px;background:#f9fafb}.status-update{background:white;padding:15px;margin:15px 0;text-align:center;border-radius:8px}.old-status{color:#6b7280;text-decoration:line-through}.new-status{color:#10b981;font-weight:bold;font-size:18px}.footer{text-align:center;padding:20px;font-size:12px;color:#6b7280}</style>
      </head>
      <body>
        <div class="container"><div class="header"><h1>Spedy Service</h1><p>Order Status Update</p></div>
        <div class="content"><h2>Dear ${order.shippingAddress?.fullName || 'Customer'},</h2>
        <p>Your order status has been updated.</p>
        <div class="status-update"><p>Status changed from: <span class="old-status">${oldStatus}</span></p><p>To: <span class="new-status">${newStatus.toUpperCase()}</span></p></div>
        <p><strong>Order ID:</strong> ${order._id}</p><p>${message}</p>
        ${order.adminNotes ? `<p><strong>Admin Notes:</strong> ${order.adminNotes}</p>` : ''}
        </div><div class="footer"><p>&copy; ${new Date().getFullYear()} Spedy Service</p></div></div>
      </body>
      </html>
    `;
    try {
